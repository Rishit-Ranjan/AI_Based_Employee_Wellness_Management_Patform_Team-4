...
import { sendAiChatMessage, fetchAiInsights, generateAiRoutine, triggerSos, fetchSystemVitals, deleteOwnAccount } from '../services/api';
...
export default function UserDashboard({
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Account (and all associated data) has been removed — log out to return to the login screen
      onLogout();
    } catch (err) {
      setDeleteAccountError(err?.message || 'Failed to delete your account. Please try again.');
    ...
    <div className="h-screen overflow-hidden bg-(--color-bg-subtle) dark:bg-(--color-bg-subtle-dark) text-(--color-text-primary) dark:text-(--color-text-primary-dark) flex flex-col font-sans transition-colors duration-300">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          />
          <div className="relative w-full max-w-md bg-(--color-bg-card) dark:bg-(--color-bg-card-dark) border border-(--color-border) dark:border-(--color-border-dark) rounded-2xl shadow-2xl p-6">
            <div className="flex items-start gap-4">
              <div className="shrink-0 w-11 h-11 rounded-full bg-red-50 dark:bg-red-950/60 flex items-center justify-center">
                <ShieldAlert className="w-5 h-5 text-red-600 dark:text-red-400" />
              </div>
              <div>
                <h3 className="text-lg font-display font-semibold text-(--color-text-primary) dark:text-(--color-text-primary-dark)">Delete Account?</h3>
                <p className="mt-2 text-sm text-(--color-text-secondary) dark:text-(--color-text-secondary-dark) leading-relaxed">
                  ...
                  and <span className="font-semibold text-red-600 dark:text-red-400">all your associated data</span> — health records, goals,
            ...
            {deleteAccountError && (
              <p className="mt-3 text-xs text-red-600 dark:text-red-400 flex items-center gap-1.5">
                <AlertCircle className="w-3.5 h-3.5" /> {deleteAccountError}
              </p>
            )}
            <div className="mt-5 pt-4 border-t border-(--color-border) dark:border-(--color-border-dark) flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowDeleteConfirm(false)}
                disabled={isDeletingAccount}
                className="px-4 py-2 rounded-lg text-xs font-semibold text-(--color-text-secondary) dark:text-(--color-text-secondary-dark) bg-(--color-bg-card) dark:bg-(--color-bg-card-dark) border border-(--color-border) dark:border-(--color-border-dark) hover:bg-(--color-bg-subtle) dark:hover:bg-(--color-bg-subtle-dark) transition-colors disabled:opacity-50 cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isDeletingAccount}
                className="px-4 py-2 rounded-lg text-xs font-bold text-white bg-red-600 hover:bg-red-700 shadow-sm shadow-red-600/30 transition-colors flex items-center gap-2 disabled:opacity-60 cursor-pointer"
              >
                <Trash2 className="w-3.5 h-3.5" />
